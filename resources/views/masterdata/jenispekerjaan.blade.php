
@extends('layouts.app')
@section('contents')
<div class="container d-flex justify-content-center mt-5">
<div class="bg-light">
        <main>
        <div class="content-wrapper" >
<div class=" bg-white">
          <div class="content-wrapper">

<!--Container Main start-->
     <div class=" bg-light">
        <main>
        <!-- plus button -->
        <div class="container">
        <div class="card">
        <div class="card-header">Data Jenis Pekerjaan</div>
        <div class="">
            <div class="">
                <a class="btn btn-mat btn-primary mb-3 mt-3" data-bs-toggle="modal" data-bs-target="#createJenisPekerjaan">
                    Tambah</a>
                    <!-- end of plus button -->


    <!-- serch -->
    <div class=" text-center mt-3">
    <div class="row">
   
      <!-- show entries -->
      <div class="kiri col-md-4">
        <label class="d-flex">Show
          <select name="adminUPTD_length" aria-controls="adminUPTD" class="">
          <option value="10">10</option>
          <option value="25">25</option>
          <option value="50">50</option>
          <option value="100">100</option>
          </select> entries
        </label>
    </div>
    <!-- search button -->
    <div class="kanan col-md-3 offset-md-2 ">
    <label class="d-flex ">Search:
      <input type="search" class="" placeholder="" aria-controls="adminUPTD">
    </label>
    </div>
    <!-- end of search button -->
     
    </div>
    </div>
            </div>
            <!-- table pekerjaan -->
        <div class="container">
                <table border="2" class="table table-striped">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Kode Pekerjaan</th>
                            <th>Jenis Pekerjaan</th>
                            <th>Satuan</th>
                            <th style="width: 20%">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                                                <tr>
                            <td>1</td>
                            <td>1.2</td>
                            <td>Mobilisasi</td>
                            <td>LS</td>
                            <td>
                                <a class="btn btn-mat btn-primary waves-effect waves-light" data-bs-toggle="modal"
                                    data-bs-target="#updateJenisPekerjaan" onclick="updateJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                                </svg>
                                    <i class="bx bx-edit-alt"></i></a>

                                <a class="btn btn-mat btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#deleteJenisPekerjaan" 
                                    onclick="deleteJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
                                </svg>
                                    <i class="bx bx-trash"></i>
                                </a>
                            </td>
                        </tr>



                        <tr>
                            <td>2</td>
                            <td>1.2</td>
                            <td>Mobilisasi</td>
                            <td>LS</td>
                            <td>
                                <a class="btn btn-mat btn-primary waves-effect waves-light" data-bs-toggle="modal"
                                    data-bs-target="#updateJenisPekerjaan" 
                                    data-jenis_pekerjaan="Mobilisasi"
                                    data-satuan="LS" onclick="updateJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                                </svg>
                                    <i class="bx bx-edit-alt"></i></a>

                                <a class="btn btn-mat btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#deleteJenisPekerjaan" 
                                    onclick="deleteJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
                                </svg>
                                    <i class="bx bx-trash"></i>
                                </a>
                            </td>
                        </tr>
          


                        <tr>
                            <td>3</td>
                            <td>1.2</td>
                            <td>Mobilisasi</td>
                            <td>LS</td>
                            <td>
                                <a class="btn btn-mat btn-primary waves-effect waves-light" data-bs-toggle="modal"
                                    data-bs-target="#updateJenisPekerjaan" 
                                    data-jenis_pekerjaan="Mobilisasi"
                                    data-satuan="LS" onclick="updateJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                                </svg>
                                    <i class="bx bx-edit-alt"></i></a>

                                <a class="btn btn-mat btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#deleteJenisPekerjaan" 
                                    onclick="deleteJenisPekerjaan(this)">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
                                </svg>
                                    <i class="bx bx-trash"></i>
                                </a>
                            </td>
                        </tr>
          
                   
                        </tbody>
                </table>
            </div>
        </div>
    </div>
       
</div>
<!-- end of table pekerjaan -->
<!-- Modal Tambah Jenis Pekerjaan -->
<div class="modal fade text-dark" id="createJenisPekerjaan">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="createJenisPekerjaanLabel">
                    Tambah Data Jenis Pekerjaan
                </h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
          
                <input type="hidden">         
                       <div class="modal-body">
                    <div class="mb-3">
                        <label  class="form-label">Kode Pekerjaan
                        </label>
                        <input type="text" class="form-control" />
                    </div>
                    <div class="mb-3">
                        <label  class="form-label">Jenis Pekerjaan
                        </label>
                        <input type="text" class="form-control"  />
                    </div>
                    <div class="mb-3">
                        <label  class="form-label">Satuan </label>
                        <input type="text" class="form-control" />
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                        Close
                    </button>
                    <button type="submit" class="btn btn-success">Save</button>
                </div>
            </form>
        </div>
    </div>
</div>

<!-- Modal Edit Jenis Pekerjaan -->
<div class="modal fade text-dark" id="updateJenisPekerjaan" >
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" >
                    Edit Data Jenis Pekerjaan
                </h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <input type="hidden">
                  <div class="modal-body">
                    <div class="mb-3">
                        <label  class="form-label">Kode Pekerjaan
                        </label>
                        <input type="text" class="form-control" />
                    </div>
                    <div class="mb-3">
                        <label  class="form-label">Jenis Pekerjaan
                        </label>
                        <input type="text" class="form-control"  />
                    </div>
                    <div class="mb-3">
                        <label  class="form-label">Satuan </label>
                        <input type="text" class="form-control" />
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                        Close
                    </button>
                    <button type="submit" class="btn btn-success">Save</button>
                </div>
            </form>
        </div>
    </div>
</div>
<!-- Modal Delete Jenis Pekerjaan -->
<div class="modal fade text-dark" id="deleteJenisPekerjaan">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="deleteJenisPekerjaanLabel">
                    Delete Data Jenis Pekerjaan
                </h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p>Apakah anda yakin ingin menghapus data ini?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                    Close
                </button>
                <a class="btn btn-danger">Delete</a>
            </div>
        </div>
    </div>
</div>
        </main>
<!-- end of main -->
</div>
    </div>
    </div>
    </div>
        </main>
</div>
</div>
@endsection